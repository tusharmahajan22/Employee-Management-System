...
void viewEmployee()
{
    ...
    while (fread (&addEmployeeInfoInDataBase, sizeof(addEmployeeInfoInDataBase), 1, fp))
    {
        ...
        printf("\t\t\tEmployee Dearness Salary = %.2f\n",(addEmployeeInfoInDataBase.employeeSalary*12.0)/100.0);
        printf("\t\t\tFather Name = %s",addEmployeeInfoInDataBase.fatherName);
...
void init()
{
    FILE *fp = NULL;
    int status = 0;
    const char defaultUsername[] ="tushar\n";
    const char defaultPassword[] ="tushar\n";
    sFileHeader fileHeaderInfo = {0};
